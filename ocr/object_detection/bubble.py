...
def test_net(model=None, image=None, params=None, bg=None, cls=None):
    ...
    if opt.TEST_BBOX_REG:
        ...
        if opt.TRAIN_BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            if opt.cuda:
                box_deltas = (
                    box_deltas.view(-1, 4)
                    * torch.FloatTensor(opt.TRAIN_BBOX_NORMALIZE_STDS).cuda()
                    + torch.FloatTensor(opt.TRAIN_BBOX_NORMALIZE_MEANS).cuda()
                )
            else:
                box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor(
                    opt.TRAIN_BBOX_NORMALIZE_STDS
    ...
    for j in range(1, len(label)):
        ...
        if inds.numel() > 0:
            ...
            demo, image, bubbles, dets_bubbles = bubble_utils.get_cnt_bubble(
                image, image.copy(), label[j], cls_dets, cls, bg=bg
            )
