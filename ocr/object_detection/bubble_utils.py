...
def draw_bubble_contour(img, class_name, score, new_contours, maxIdx, idx):
    if opt.DRAWBUB is True:
        ...
        cv2.putText(
            img,
            "{:s}_{:d}: {:.3f}".format(class_name, idx, score),
            (0, 10),
...
def adjust_bbox_coord(img, xmin, ymin, xmax, ymax, bg="white"):
    ...
    if bg == "white" or "black":
        ...
        thresh = cv2.copyMakeBorder(
            thresh[0:-8, 0:-8], 0, 8, 0, 8, borderType=cv2.BORDER_CONSTANT
        )
...
def get_cnt_bubble(demo, image, class_name, dets, class_thresh, bg="white"):
    ...
    try:
        for bub_order in order_k:
            ...
            crop_tmp = crop.copy()
            cnts, _ = cv2.findContours(thresh, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            max_idx, max_area = find_max_contour(contours=cnts)
            crop_tmp = draw_bubble_contour(
                crop_tmp, class_name, scores[bub_order], cnts, max_idx, count
            )
