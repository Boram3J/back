...
def gen_txt_to_image(load_from=None, warp_item=None):
    ...
    for k, text in enumerate(labels):
        try:
            ...
            if (
                len(line_boxes_coordinate) == 0
            ):  # It doesn't exist detected line text, but bubbles
                item_idx += 1
                continue
            ...
            for x, slices in enumerate(arr):
                ...
                font = ImageFont.truetype(str(curpath / "font.ttf"), fontsize)
                ...
                if bubble_image.shape[1] - orig_xmax >= opt.WARP_SPACE_THRESHOLD:
                    img = cv2.resize(
                        img,
                        (orig_xmax - orig_xmin, orig_ymax - orig_ymin),
                    ...
                    img = cv2.resize(
                        img,
                        (tmp_xmax - tmp_xmin, tmp_ymax - tmp_ymin),
                        interpolation=cv2.INTER_CUBIC,
                    )
            ...
            for leaving in leavings:
                ...
                xmax, ymax = leaving[2]
                cv2.rectangle(
                    bubble_image, (xmin, ymin), (xmax, ymax), (255, 255, 255), -1
                )
            cnt = 0
